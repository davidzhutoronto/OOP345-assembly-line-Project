...
namespace sdds {
	...
	{
		throw "Copy Constructor is not allowed in this class.";
	}
	...
	bool CustomerOrder::isItemFilled(const std::string& itemName) const
	{
		/*bool flag = true;
		for (size_t i = 0u; i < m_cntItem; i++) {
			if (itemName == m_lstItem[i]->m_itemName && m_lstItem[i]->m_isFilled) {
				flag = true;
			}
			else if (itemName == m_lstItem[i]->m_itemName && !m_lstItem[i]->m_isFilled) {
				flag = false;
			}
		}
